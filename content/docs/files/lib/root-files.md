...
| File | Description |
|------|-------------|
| `main.dart` | App entry point (~140 lines). Calls `AppInitializationService` for startup, renders `AuthGate` which delegates to `SessionManagerService` for auth state. Most logic was extracted to 4 dedicated services in February 2026 (previously ~845 lines). |
...
| `platform_config.dart` | Compile-time platform flags (`kPlatformMobile`, `kPlatformDesktop`) for tree-shaking. Auto-detects when not explicitly set. The `FEATURE_IMAGE_GEN` flag was removed (image generation is now always enabled). |
